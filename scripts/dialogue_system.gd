...
func setup_example_dialogue():
	...
	var start = DialogueNode.new("start", "Old Miner", 
		"Well, well... if it isn't the hero who saved us all. What brings you to these parts?")
	start.add_option(DialogueOption.new("I'm looking for information about the aftermath.", "aftermath"))
	start.add_option(DialogueOption.new("[Rhetoric] Convince him you need his help.", "persuade", "rhetoric", 8))
	start.add_option(DialogueOption.new("[Perception] Notice something off about him.", "perceive", "perception", 7))
	start.add_option(DialogueOption.new("Just passing through.", "end"))
	dialogue["start"] = start
	
	var aftermath = DialogueNode.new("aftermath", "Old Miner",
	...
	var persuade_success = DialogueNode.new("persuade_success", "Old Miner",
	...
	var persuade_fail = DialogueNode.new("persuade_fail", "Old Miner",
	...
	var perceive_success = DialogueNode.new("perceive_success", "Old Miner",
		"[You notice his hands trembling, and dried blood under his fingernails. He's hiding something.]")
	perceive_success.add_option(DialogueOption.new("[Authority] Tell me what you're hiding. Now.", "details", "authority", 8))
	perceive_success.add_option(DialogueOption.new("Let it go.", "end"))
	dialogue["perceive_success"] = perceive_success
	
	var perceive_fail = DialogueNode.new("perceive_fail", "Old Miner",
		"[He seems like a typical old miner. Nothing unusual.]")
