...
## Checks if a memory flag exists in world-level flags or NPC-specific flags
## Returns true if found, false otherwise
static func has_memory_flag(flag_name: String) -> bool:
	var world = GameState.state.get("world", {})

	# Check world-level memory flags first
	if world.get("memory_flags", []).has(flag_name):
		return true

	# Check NPC-specific flags for backward compatibility
	var npc_states = world.get("npc_states", {})
