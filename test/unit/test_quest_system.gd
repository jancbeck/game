...
func test_violent_approach_degrades_charisma_and_adds_conviction():
	...
	# Prerequisites are checked in data loader validation but for pure system test
	# we assume the quest is active in state.
...
func test_stealthy_approach_degrades_cunning_and_adds_conviction():
	...
	assert_that(result["player"]["convictions"]["deceptive_acts"]).is_equal(2)  # 0 + 2
...
func test_memory_flags_set_correctly_for_violent_approach():
	...
	# Assert
	assert_that(result["world"]["npc_states"]["guard_captain"]["memory_flags"]).contains("hostile")
