...
func _ready() -> void:
	# Create room music player
	room_music = AudioStreamPlayer.new()
	room_music.stream = load("res://src/world/room/music_room_entered_loop.wav")
	room_music.bus = "Music"
	room_music.volume_db = 0.0
	room_music.finished.connect(_on_room_music_finished)
	add_child(room_music)

	# Create aggro music player
	aggro_music = AudioStreamPlayer.new()
	aggro_music.stream = load("res://src/world/room/music_enemies_aggro_loop.wav")
	aggro_music.bus = "Music"
