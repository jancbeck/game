...
@export var bullet_scene: PackedScene = preload("res://src/items/bullet/Bullet.tscn")
...
func _ready() -> void:
	...
	tex = load("res://src/actors/player/player_8dir.png")
	$"Sprite".texture = tex
...
func _shoot() -> void:
	...
	# Very light screen shake for shooting (reduced to prevent spam accumulation)
	ScreenShake.add_trauma(0.08)
