...
func test_is_mockable_by_script_path() -> void:
	(
		assert_that(GdUnitMockBuilder.is_mockable(resource_path + "CustomResourceTestClass.gd"))
		. is_true()
	)
...
func test_mock_godot_class_fullcheck(
	fuzzer := GodotClassNameFuzzer.new(), fuzzer_iterations := 200
) -> void:
...
func test_mock_scene_variables_is_set() -> void:
	...
	assert_bool(
		mocked_scene.is_connected(
			"panel_color_change", Callable(mocked_scene, "_on_panel_color_changed")
		)
	)
