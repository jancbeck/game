...
func test_extract_charset() -> void:
	...
	assert_str(StringFuzzer.extract_charset("a-z").get_string_from_utf8()).is_equal(
	...
	assert_str(StringFuzzer.extract_charset("-a-dA-D2-8+_").get_string_from_utf8()).is_equal(
		"-abcdABCD2345678+_"
	)
	# range token at end
	assert_str(StringFuzzer.extract_charset("a-dA-D2-8+_-").get_string_from_utf8()).is_equal(
	...
	assert_str(StringFuzzer.extract_charset("a-d-A-D2-8+_").get_string_from_utf8()).is_equal(
		"abcd-ABCD2345678+_"
	)
