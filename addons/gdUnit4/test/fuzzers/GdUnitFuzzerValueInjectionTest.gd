...
func test_fuzzer_has_same_instance_peer_iteration(
	fuzzer := TestFuzzer.new(), fuzzer_iterations := 10
) -> void:
...
func test_fuzzer_iterations_custom_value(
	fuzzer := Fuzzers.rangei(-23, 22), fuzzer_iterations := 234, fuzzer_seed := 100
) -> void:
...
func test_fuzzer_with_timeout(
	fuzzer := Fuzzers.rangei(-23, 22), fuzzer_iterations := 20, timeout := 100
) -> void:
...
func test_fuzzer_inject_value_with_seed(
	fuzzer := Fuzzers.rangei(-23, 22), fuzzer_iterations := 10, fuzzer_seed := 187772
) -> void:
	assert_object(fuzzer).is_not_null()
