...
func after() -> void:
	...
	assert_array(_events).extractv(extr("type"), extr("is_skipped"), extr("test_name")).contains(
		[tuple(GdUnitEvent.TESTCASE_AFTER, true, "test_unknown_argument_in_test_case")]
	)
