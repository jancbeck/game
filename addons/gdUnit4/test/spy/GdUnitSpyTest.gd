...
func test_spy_source_with_class_name_by_resource_path() -> void:
	var instance: Object = auto_free(
		load("res://addons/gdUnit4/test/mocker/resources/GD-256/world.gd").new()
	)
	var m: Variant = spy(instance)
	var head: String = m.get_script().source_code.substr(0, 200)
	assert_str(head).contains("class_name DoubledMunderwoodPathingWorld").contains(
...
func test_spy_extends_godot_class() -> void:
	...
	var head: String = m.get_script().source_code.substr(0, 200)
	assert_str(head).contains("class_name DoubledWorld").contains("extends World3D")
...
func test_spy_class_with_custom_formattings() -> void:
	...
	assert_failure(func() -> void: verify_no_interactions(do_spy)).is_failed().has_line(117)


func test_spy_copied_class_members() -> void:
	var instance: Object = auto_free(
		load("res://addons/gdUnit4/test/mocker/resources/TestPersion.gd").new(
			"user-x", "street", 56616
		)
	)
...
func test_spy_snake_case_named_class_by_resource_path() -> void:
	...
	var instance_b: Object = (
		load("res://addons/gdUnit4/test/mocker/resources/snake_case_class_name.gd").new()
	)
...
func test_spy_scene_initalize() -> void:
	...
	assert_bool(
		spy_scene.is_connected("panel_color_change", Callable(spy_scene, "_on_panel_color_changed"))
	)
