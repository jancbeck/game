...
func test_commands() -> void:
	assert_array(CmdOption.new("-a", "help a", "describe a").commands()).contains_exactly(["-a"])
	assert_array(CmdOption.new("-a, --aaa", "help a", "describe a").commands()).contains_exactly(
		["-a", "--aaa"]
	)
	# containing space or tabs
	(
		assert_array(CmdOption.new("-b ,  --bb	", "help a", "describe a").commands())
		. contains_exactly(["-b", "--bb"])
	)
...
func test_description() -> void:
	assert_str(CmdOption.new("-a, --aaa", "help a", "describe a").description()).is_equal(
		"describe a"
	)
