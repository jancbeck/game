...
func test_progress_success() -> void:
	...
	for index in 10:
		_runner.invoke(
			...
			GdUnitEvent.new().test_after(
				"res://test/testA.gd", "TestSuiteA", "test_parameterized:%d (params)" % index, {}
			)
		)
		assert_that(_progress.value).is_equal(expected_progess_index)
	...
	_runner.invoke(
		"_on_gdunit_event",
		GdUnitEvent.new().test_statistics(
			"res://test/testA.gd", "TestSuiteA", "test_parameterized", {}
		)
	)
...
func test_progress_failed(
	...
	_runner.invoke(
		"_on_gdunit_event",
		GdUnitEvent.new().test_statistics(
			"res://test/testA.gd", "TestSuiteA", test_name, statistics
		)
	)
