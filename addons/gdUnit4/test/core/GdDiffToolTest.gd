...
func test_string_diff() -> void:
	...
	var ord_b := chars[2]
	var ord_c := chars[3]
	var expected_l_diff := PackedByteArray(
		[GdDiffTool.DIV_SUB, ord_A, GdDiffTool.DIV_ADD, ord_a, ord_b, ord_c]
	)
	var expected_r_diff := PackedByteArray(
		[GdDiffTool.DIV_ADD, ord_A, GdDiffTool.DIV_SUB, ord_a, ord_b, ord_c]
	)
...
func test_string_diff_large_value(
	fuzzer := Fuzzers.rand_str(1000, 4000), fuzzer_iterations := 10
) -> void:
