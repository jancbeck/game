...
func test_add_test_case() -> void:
	...
	assert_dict(config.to_execute()).contains_key_value(
		"res://foo1", PackedStringArray(["testcaseA"])
	)
	# add two more
...
func test_skip_test_case() -> void:
	...
	assert_dict(config.skipped()).contains_key_value(
		"res://foo1.gd", PackedStringArray(["testcaseA"])
	)
	# add two more
...
func test_load_fail() -> void:
	...
	assert_result(config.load_config("invalid_path")).is_error().contains_message(
...
func test_load_example_config() -> void:
	var config := GdUnitRunnerConfig.new()
	config.load_config(
		"res://addons/gdUnit4/test/core/resources/GdUnitRunner_with_skipped_testsuites.cfg"
	)

	assert_dict(config.to_execute()).has_size(1).contains_key_value(
		"res://addons/gdUnit4/test/asserts/", PackedStringArray()
	)
	(
		assert_dict(config.skipped())
		. has_size(4)
		. contains_key_value(
			"res://addons/gdUnit4/test/asserts/GdUnitFailureAssertImplTest.gd", PackedStringArray()
		)
		. contains_key_value(
			"res://addons/gdUnit4/test/asserts/GdUnitAssertImplTest.gd", PackedStringArray()
		)
		. contains_key_value(
			"res://addons/gdUnit4/test/asserts/GdUnitIntAssertImplTest.gd", PackedStringArray()
		)
		. contains_key_value(
			"res://addons/gdUnit4/test/asserts/GdUnitVectorAssertImplTest.gd", PackedStringArray()
		)
	)
