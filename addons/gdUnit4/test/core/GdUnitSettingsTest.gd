...
func test_list_settings() -> void:
	...
	(
		assert_array(settingsA)
		. extractv(extr("name"), extr("type"), extr("value"), extr("default"), extr("help"))
...
func test_enum_property() -> void:
	...
	GdUnitSettings.create_property_if_need(
		"test/enum", GdUnitSettings.NAMING_CONVENTIONS.AUTO_DETECT, "help", value_set
	)
