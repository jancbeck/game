...
func test_parameterized_bool_value(
	a: int, expected: bool, test_parameters := [[0, false], [1, true]]
) -> void:
...
func test_parameterized_int_values(
	...
	c: int,
	expected: int,
	test_parameters := [[1, 2, 3, 6], [3, 4, 5, 12], [6, 7, 8, 21]]
) -> void:
...
func test_parameterized_float_values(
	...
	b: float,
	expected: float,
	test_parameters := [[2.2, 2.2, 4.4], [2.2, 2.3, 4.5], [3.3, 2.2, 5.5]]
) -> void:
...
func test_parameterized_string_values(
	...
	b: String,
	expected: String,
	test_parameters := [["2.2", "2.2", "2.22.2"], ["foo", "bar", "foobar"], ["a", "b", "ab"]]
) -> void:
...
@warning_ignore("unused_parameter")
func test_parameterized_untyped_array(items: Array, test_parameters := [[[42]]]) -> void:
	collect_test_call("test_parameterized_untyped_array", [items])
	assert_array(items).contains_exactly([42])


@warning_ignore("unused_parameter")
func test_parameterized_typed_array(items: Array[int], test_parameters := [[[42]]]) -> void:
...
func test_with_extern_const_parameter_set(
	value: String, test_parameters := [_data1, _data2]
) -> void:
	assert_that(value).is_not_empty()
	assert_that(test_parameters).is_empty()
