...
func assert_event_list(
	...
	expected_events.append(
		tuple(GdUnitEvent.TESTSUITE_BEFORE, suite_name, "before", test_case_names.size())
	)
...
func assert_test_statistics(events: Array[GdUnitEvent]) -> GdUnitArrayAssert:
	var _events := events.filter(
		func(event: GdUnitEvent) -> bool: return event.type() in [GdUnitEvent.TESTCASE_STATISTICS]
	)
...
func assert_event_reports(events: Array[GdUnitEvent], expected_reports: Array) -> void:
	var _events := events.filter(
		func(event: GdUnitEvent) -> bool: return event.type() != GdUnitEvent.TESTCASE_STATISTICS
	)
...
func test_execute_success() -> void:
	...
	assert_event_list(events, "TestSuiteAllStagesSuccess", ["test_case1", "test_case2"])
...
func test_execute_failure_on_stage_before() -> void:
	...
	var events := await execute(test_suite)
	# verify basis infos
	assert_event_list(events, "TestSuiteFailOnStageBefore", ["test_case1", "test_case2"])
...
func test_execute_failure_on_stage_after() -> void:
	...
	var events := await execute(test_suite)
	# verify basis infos
	assert_event_list(events, "TestSuiteFailOnStageAfter", ["test_case1", "test_case2"])
...
func test_execute_failure_on_stage_before_test() -> void:
	...
	var events := await execute(test_suite)
	# verify basis infos
	assert_event_list(events, "TestSuiteFailOnStageBeforeTest", ["test_case1", "test_case2"])
...
func test_execute_failure_on_stage_after_test() -> void:
	...
	var events := await execute(test_suite)
	# verify basis infos
	assert_event_list(events, "TestSuiteFailOnStageAfterTest", ["test_case1", "test_case2"])
...
func test_execute_failure_on_stage_test_case1() -> void:
	...
	var events := await execute(test_suite)
	# verify basis infos
	assert_event_list(events, "TestSuiteFailOnStageTestCase1", ["test_case1", "test_case2"])
...
func test_execute_failure_on_multiple_stages() -> void:
	...
	var events := await execute(test_suite)
	# verify basis infos
	assert_event_list(events, "TestSuiteFailOnMultipeStages", ["test_case1", "test_case2"])
...
func test_execute_failure_and_orphans() -> void:
	...
	var test_suite := _load(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteFailAndOrpahnsDetected.resource"
	)
	# simulate test suite execution
	var events := await execute(test_suite)
	# verify basis infos
	assert_event_list(events, "TestSuiteFailAndOrpahnsDetected", ["test_case1", "test_case2"])
	# we expect failing on multiple stages
	(
		assert_test_counters(events)
		. contains_exactly(
...
func test_execute_error_on_test_timeout() -> void:
	...
	var events := await execute(test_suite)
	# verify basis infos
	assert_event_list(events, "TestSuiteErrorOnTestTimeout", ["test_case1", "test_case2"])
	...
	(
		...
		. contains_exactly(
			[
				tuple("before", SUCCEEDED, NOT_SKIPPED, false, false, false),
				tuple(
					"test_multi_yielding_with_fuzzer", SUCCEEDED, NOT_SKIPPED, false, false, false
				),
				tuple(
					"test_multi_yielding_with_fuzzer", SUCCEEDED, NOT_SKIPPED, false, false, false
				),
...
func test_execute_add_child_on_before_GD_106() -> void:
	...
	var events := await execute(test_suite)
	# verify basis infos
	assert_event_list(events, "TestSuiteFailAddChildStageBefore", ["test_case1", "test_case2"])
	# verify all counters are zero / no errors, failures, orphans
	(
		assert_test_counters(events)
		. contains_exactly(
		...
		. contains(
			[
				tuple(
					GdUnitEvent.TESTCASE_AFTER,
					suite_name,
					"test_dictionary_div_number_types:0",
					false,
					true,
					0
				),
				tuple(
					GdUnitEvent.TESTCASE_AFTER,
					suite_name,
					"test_dictionary_div_number_types:1",
					false,
					false,
					0
				),
				tuple(
					GdUnitEvent.TESTCASE_AFTER,
					suite_name,
					"test_dictionary_div_number_types:2",
					false,
					true,
					0
				),
				tuple(
					GdUnitEvent.TESTCASE_AFTER,
					suite_name,
					"test_dictionary_div_number_types:3",
					false,
					false,
					0
				)
...
func test_execute_test_case_is_flaky_and_failed() -> void:
	...
	(
		assert_array(events)
		. extractv(extr("test_name"), extr("is_flaky"))
		. contains(
			[
				tuple("before", false),
	...
	(
		assert_array(events)
		. extractv(
			extr("type"), extr("test_name"), extr("is_success"), extr("is_flaky"), extr("is_failed")
		)
		. contains(
			[
	...
	(
		...
		. contains_exactly(
			[
				...
				tuple(
					GdUnitEvent.TESTCASE_AFTER,
					"test_paramaterized_flaky:0 [0, 1]",
					true,
					false,
					false
				),
	...
	(
		...
		. contains_exactly(
			[
				...
				tuple(
					GdUnitEvent.TESTCASE_AFTER,
					"test_paramaterized_flaky:1 [1, 1]",
					true,
					false,
					false
				),
	...
	(
		...
		. contains_exactly(
			[
				...
				tuple(
					GdUnitEvent.TESTCASE_AFTER,
					"test_paramaterized_flaky:2 [2, 6]",
					false,
					false,
					true
				),
	...
	(
		...
		. contains_exactly(
			[
				...
				tuple(
					GdUnitEvent.TESTCASE_AFTER,
					"test_paramaterized_flaky:3 [3, 1]",
					true,
					false,
					false
				),
	...
	(
		...
		. contains_exactly(
			[
				...
				tuple(
					GdUnitEvent.TESTCASE_AFTER,
					"test_paramaterized_flaky:4 [4, 3]",
					false,
					false,
					true
				),
...
func filter_by_test_case(events: Array[GdUnitEvent], test_case_name: String) -> Array[GdUnitEvent]:
	return events.filter(
		func(event: GdUnitEvent) -> bool:
			return (
				event.test_name() == test_case_name and event.type() == GdUnitEvent.TESTCASE_AFTER
			)
	)


class TestCaseNameExtractor:
