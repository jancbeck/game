...
func test_parse_and_add_test_cases() -> void:
	...
	test_suite.set_script(
		load("res://addons/gdUnit4/test/core/resources/test_script_with_arguments.gd")
	)
		...
		. contains_exactly(
			[
				...
				tuple(
					...
					[
						GdFunctionArgument.new(
							"fuzzer", GdObjects.TYPE_FUZZER, "Fuzzers.rangei(-10, 22)"
						)
					],
					Fuzzer.ITERATION_DEFAULT_COUNT
				),
				tuple(
					"test_with_fuzzer_iterations",
					default_time,
				...
				tuple(
					...
					[
						GdFunctionArgument.new(
							"fuzzer_a", GdObjects.TYPE_FUZZER, "Fuzzers.rangei(-10, 22)"
						),
						GdFunctionArgument.new(
							"fuzzer_b", GdObjects.TYPE_FUZZER, "Fuzzers.rangei(23, 42)"
						)
					],
					10
				),
...
func test_scan_by_inheritance_class_name() -> void:
	...
	# sort by names
	test_suites.sort_custom(
		func by_name(a: GdUnitTestSuite, b: GdUnitTestSuite) -> bool:
			return a.get_name().length() <= b.get_name().length()
	)
	assert_array(test_suites).extract("get_name").contains_exactly(
		["BaseTest", "ExtendedTest", "ExtendsExtendedTest"]
	)
	(
		assert_array(test_suites)
		. extract("get_script.get_path")
		. contains_exactly(
			[
	...
	assert_array(test_suites[0].get_children()).extract("name").contains_same_exactly_in_any_order(
		[&"test_foo1"]
	)
	assert_array(test_suites[1].get_children()).extract("name").contains_same_exactly_in_any_order(
		[&"test_foo2", &"test_foo1"]
	)
	assert_array(test_suites[2].get_children()).extract("name").contains_same_exactly_in_any_order(
		[&"test_foo3", &"test_foo2", &"test_foo1"]
	)
...
func test_scan_by_inheritance_class_path() -> void:
	var scanner: GdUnitTestSuiteScanner = GdUnitTestSuiteScanner.new()
	var test_suites := scanner.scan(
		"res://addons/gdUnit4/test/core/resources/scan_testsuite_inheritance/by_class_path/"
	)

	(
		assert_array(test_suites)
		. extractv(extr("get_name"), extr("get_script.get_path"), extr("get_children.get_name"))
		. contains_exactly_in_any_order(
			[
				tuple(
					"BaseTest",
					"res://addons/gdUnit4/test/core/resources/scan_testsuite_inheritance/by_class_path/BaseTest.gd",
					[&"test_foo1"]
				),
				tuple(
					"ExtendedTest",
					"res://addons/gdUnit4/test/core/resources/scan_testsuite_inheritance/by_class_path/ExtendedTest.gd",
					[&"test_foo2", &"test_foo1"]
				),
				tuple(
					"ExtendsExtendedTest",
					"res://addons/gdUnit4/test/core/resources/scan_testsuite_inheritance/by_class_path/ExtendsExtendedTest.gd",
					[&"test_foo3", &"test_foo2", &"test_foo1"]
				)
			]
		)
...
func test_scan_test_suite_without_tests() -> void:
	var scanner: GdUnitTestSuiteScanner = GdUnitTestSuiteScanner.new()
	var test_suites := scanner.scan(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteWithoutTests.gd"
	)
