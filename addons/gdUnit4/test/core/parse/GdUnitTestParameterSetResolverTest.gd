...
func test_example_b(
	a: Vector2,
	b: Vector2,
	test_parameters := [[Vector2.ZERO, Vector2.ONE], [Vector2(1.1, 3.2), Vector2.DOWN]]
) -> void:
	pass
...
func test_example_c(
	a: Object,
	b: Object,
	test_parameters := [[Resource.new(), Resource.new()], [Resource.new(), null]]
) -> void:
	pass


@warning_ignore("unused_parameter")
func test_resolve_parameters_static(a: int, b: int, test_parameters := [[1, 10], [2, 20]]) -> void:
	pass


@warning_ignore("unused_parameter")
func test_resolve_parameters_at_runtime(
	a: int, b: int, test_parameters := [[1, _test_param1], [2, _test_param2], [3, 30]]
) -> void:
	pass
...
func test_example_d(
	a: Vector3,
	b: Vector3,
	test_parameters := [[build_param(1), build_param(3)], [Vector3.BACK, Vector3.UP]]
) -> void:
	pass
...
func test_example_e(
	...
	b: Object,
	expected: String,
	test_parameters := [[TestObj.new("abc"), TestObj.new("def"), "abcdef"]]
) -> void:
	pass
...
func test_load_parameter_sets() -> void:
	...
	assert_array(tc.parameter_set_resolver().load_parameter_sets(tc)).is_equal([[1, 2], [3, 4]])

	tc = get_test_case("test_example_b")
	assert_array(tc.parameter_set_resolver().load_parameter_sets(tc)).is_equal(
...
func test_build_test_case_names_on_static_parameter_set() -> void:
	...
	assert_array(resolver.build_test_case_names(test_case)).contains_exactly(
		["test_resolve_parameters_static:0 [1, 10]", "test_resolve_parameters_static:1 [2, 20]"]
	)
