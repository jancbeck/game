...
static func load_cs_script(resource_path: String, debug_write := false) -> Script:
	...
	if debug_write:
		script_resource_path = (
			GdUnitFileAccess.create_temp_dir("test") + "/%s" % script_resource_path.get_file()
		)
		...
		if err != OK:
			print_debug(
				"Can't save debug resource", script_resource_path, "Error:", error_string(err)
			)
...
static func load_gd_script(resource_path: String, debug_write := false) -> GDScript:
	...
	if debug_write:
		...
		var err := ResourceSaver.save(
			script, script_resource_path, ResourceSaver.FLAG_REPLACE_SUBRESOURCE_PATHS
		)
		if err != OK:
			print_debug(
				"Can't save debug resource", script_resource_path, "Error:", error_string(err)
