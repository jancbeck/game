...
func test_is_success() -> void:
	...
	var assert_ := await assert_failure_await(
		func() -> void:
...
func test_is_assert_failed() -> void:
	await (
		assert_error(func() -> void: await GodotErrorTestClass.new().test(1))
	...
	var assert_ := await assert_failure_await(
		func() -> void:
			await (
				assert_error(func() -> void: GodotErrorTestClass.new().test(0))
				. is_runtime_error("Assertion failed: this is an assert error")
			)
	)
	(
...
func test_is_push_warning() -> void:
	await (
		assert_error(func() -> void: GodotErrorTestClass.new().test(2))
...
func test_is_push_error() -> void:
	await (
		assert_error(func() -> void: GodotErrorTestClass.new().test(3))
...
func test_is_runtime_error() -> void:
	await (
		assert_error(func() -> void: GodotErrorTestClass.new().test(4))
