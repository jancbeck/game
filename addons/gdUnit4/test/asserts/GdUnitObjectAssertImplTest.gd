...
func test_is_equal() -> void:
	...
	assert_failure(func() -> void: assert_object(Mesh.new()).is_equal(Skin.new())).is_failed()
	(
...
func test_is_not_equal() -> void:
	...
	assert_failure(func() -> void: assert_object(Mesh.new()).is_not_equal(Mesh.new())).is_failed()
...
func test_is_not_instanceof() -> void:
	...
	assert_object(auto_free(CustomClass.InnerClassB.new())).is_not_instanceof(
		CustomClass.InnerClassC
	)

	(
		assert_failure(
			func() -> void: assert_object(auto_free(Path3D.new())).is_not_instanceof(Node)
		)
		. is_failed()
...
func test_is_same() -> void:
	...
	assert_failure(func() -> void: assert_object(obj1).is_same(obj3)).is_failed()
	assert_failure(func() -> void: assert_object(obj3).is_same(obj1)).is_failed()
	assert_failure(func() -> void: assert_object(obj3).is_same(obj2)).is_failed()
...
func test_override_failure_message() -> void:
	(
		assert_object(assert_object(auto_free(Node.new())).override_failure_message("error"))
		. is_instanceof(GdUnitObjectAssert)
	)
	(
		assert_failure(
			func() -> void:
				(
					assert_object(auto_free(Node.new()))
...
func test_append_failure_message() -> void:
	(
		assert_object(assert_object(auto_free(Node.new())).append_failure_message("error"))
		. is_instanceof(GdUnitObjectAssert)
	)
	(
		assert_failure(
...
func test_is_failure() -> void:
	...
	assert_failure(func() -> void: assert_object(RefCounted.new()).is_null()).is_failed()
