	...
	if not keys_not_found.is_empty():
		return report_error(
			GdAssertMessages.error_contains_keys(
				current.keys(), expected, keys_not_found, compare_mode
			)
		)
	return report_success()
...
func _contains_key_value(
	...
	if not GdObjects.equals(current[key], value, false, compare_mode):
		return report_error(
			GdAssertMessages.error_contains_key_value(key, value, current[key], compare_mode)
		)
	return report_success()
	...
	if not keys_found.is_empty():
		return report_error(
			GdAssertMessages.error_not_contains_keys(
				current.keys(), expected, keys_found, compare_mode
			)
		)
	return report_success()
