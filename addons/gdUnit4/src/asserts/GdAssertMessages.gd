	...
	if event is InputEventWithModifiers:
		text += (
			...
			% [
				...
				event.ctrl_pressed,
				event.meta_pressed,
				event.command_or_control_autoremap
			]
		)
	return text
...
static func test_suite_skipped(hint: String, skip_count: int) -> String:
	return (
		...
		% [
			_error("Entire test-suite is skipped!"),
...
static func error_arr_contains(
	current: Variant, expected: Array, not_expect: Array, not_found: Array, by_reference: bool
) -> String:
	var failure_message := (
...
static func error_arr_not_contains(
	current: Array, expected: Array, found: Array, compare_mode: GdObjects.COMPARE_MODE
) -> String:
	var failure_message := (
		"Expecting:"
		if compare_mode == GdObjects.COMPARE_MODE.PARAMETER_DEEP_TEST
...
static func error_validate_interactions(
	...
	for args: Variant in current_interactions.keys():
		...
		interactions.append(_format_arguments(args, times))
	var expected_interaction := _format_arguments(
		expected_interactions.keys()[0], expected_interactions.values()[0]
	)
	return (
		"%s\n%s\n%s\n%s"
		% [
...
static func format_chars(characters: PackedByteArray, type: Color) -> PackedByteArray:
	...
	var result := PackedByteArray()
	var message := (
		"[bgcolor=#%s][color=with]%s[/color][/bgcolor]"
