...
func _contains_exactly(
	...
	var diffs := _array_div(
		compare_mode, current_value, expected, GdObjects.COMPARE_MODE.PARAMETER_DEEP_TEST
	)
...
func extract(func_name: String, args := Array()) -> GdUnitArrayAssert:
	...
	var extractor: GdUnitValueExtractor = (
		ResourceLoader
		. load(
			"res://addons/gdUnit4/src/extractors/GdUnitFuncValueExtractor.gd",
			"GDScript",
			ResourceLoader.CACHE_MODE_REUSE
		)
		. new(func_name, args)
