...
func update_testsuite_counters(
	...
	update_summary_counters(
		p_error_count, p_failure_count, p_orphan_count, p_is_skipped, p_is_flaky, 0
	)
...
func write() -> String:
	var template := GdUnitHtmlPatterns.load_template(
		"res://addons/gdUnit4/src/report/template/index.html"
	)
	...
	GdUnitFileAccess.copy_directory(
		"res://addons/gdUnit4/src/report/template/css/", _report_path + "/css"
	)
	return index_file
...
func delete_history(max_reports: int) -> int:
	return GdUnitFileAccess.delete_path_index_lower_equals_than(
		_report_path.get_base_dir(), REPORT_DIR_PREFIX, _iteration - max_reports
	)
