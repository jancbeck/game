		...
		if report.is_failure():
			failure_reports.append(
				(
					...
					. attribute(
						ATTR_MESSAGE,
						"FAILED: %s:%d" % [test_report._resource_path, report.line_number()]
					)
			...
			failure_reports.append(
				(
					...
					. attribute(
						ATTR_MESSAGE,
						"ERROR: %s:%d" % [test_report._resource_path, report.line_number()]
					)
					. attribute(ATTR_TYPE, JUnitXmlReport.to_type(report.type()))
					. text(convert_rtf_to_text(report.message()))
				)
			...
			failure_reports.append(
				(
					...
					. attribute(
						ATTR_MESSAGE,
						"SKIPPED: %s:%d" % [test_report._resource_path, report.line_number()]
					)
					. text(convert_rtf_to_text(report.message()))
				)
			)
