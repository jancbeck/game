...
func __get_mocked_return_value_or_default(
...
func __do_call_real_func(__func_name: String, __func_args := []) -> bool:
	var __is_call_real_func := (
		__mock_working_mode == GdUnitMock.CALL_REAL_FUNC and not __excluded_methods.has(__func_name)
	)
