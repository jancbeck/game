...
func _handle_test_suite_arguments(
	test_suite: Node, script: GDScript, fd: GdFunctionDescriptor
) -> void:
	for arg in fd.args():
		match arg.name():
...
func _handle_test_case_arguments(
	test_suite: Node, script: GDScript, fd: GdFunctionDescriptor
) -> void:
	...
	# create new test
	test.configure(
		fd.name(), fd.line_number(), fd.source_path(), timeout, fuzzers, iterations, seed_value
	)
...
func _parse_and_add_test_cases(
	test_suite: Node, script: GDScript, test_case_names: PackedStringArray
) -> void:
