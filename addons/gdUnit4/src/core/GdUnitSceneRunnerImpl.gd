...
var GdUnitFuncAssertImpl := ResourceLoader.load(
...
func simulate_screen_touch_release(index: int, double_tap := false) -> GdUnitSceneRunner:
	...
	event.pressed = false
	event.double_tap = (
		_last_input_event.double_tap if _last_input_event is InputEventScreenTouch else double_tap
	)
	_current_scene.get_viewport().push_input(event)
	return self
...
func simulate_screen_touch_drag_relative(
	...
	return await _do_touch_drag_at(
		index, _current_touch_position[index] + relative, time, trans_type
	)
