...
func discover(script: Script) -> void:
	...
	if GdObjects.is_test_suite(script):
		...
		if not _discover_cache.has(script_path):
			...
			GdUnitSignals.instance().gdunit_event.emit(
				GdUnitEventTestDiscoverTestSuiteAdded.new(script_path, suite_name, dto)
			)
			sync_cache(dto)
			test_suite.queue_free()
			return

		var discovered_test_cases: Array[String] = _discover_cache.get(
			script_path, [] as Array[String]
		)
		...
		if not tests_removed.is_empty() or not tests_added.is_empty():
			...
			for test_name in tests_removed:
				...
				GdUnitSignals.instance().gdunit_event.emit(
					GdUnitEventTestDiscoverTestRemoved.new(script_path, suite_name, test_name)
				)
			...
			for test_name in tests_added:
				...
				GdUnitSignals.instance().gdunit_event.emit(
					GdUnitEventTestDiscoverTestAdded.new(script_path, suite_name, dto)
				)
			# update the cache
