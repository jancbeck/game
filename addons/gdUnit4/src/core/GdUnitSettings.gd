	...
	for property in ProjectSettings.get_property_list():
		...
		if property_name == name:
			...
			return GdUnitProperty.new(
				property_name, property["type"], value, default, extract_help_text(help), value_set
			)
	return null
...
static func migrate_property(
	...
	prints(
		"Succesfull migrated property '%s' -> '%s' value: %s" % [old_property, new_property, value]
	)
