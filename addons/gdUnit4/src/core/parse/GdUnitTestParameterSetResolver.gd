...
const EXCLUDE_PROPERTIES_TO_COPY = ["script", "type", "Node", "_import_path"]
		...
		if input_values is Array:
			...
			if current_arg_count != expected_arg_count:
				return (
					...
					% [parameter_set_index, expected_arg_count, current_arg_count]
				)
			var error := GdUnitTestParameterSetResolver.validate_parameter_types(
				input_arguments, input_values, parameter_set_index
			)
...
static func validate_parameter_types(
	input_arguments: Array, input_values: Array, parameter_set_index: int
) -> String:
...
func build_test_case_names(test_case: _TestCase) -> PackedStringArray:
	...
	if parameter_sets.size() == 0:
		...
		for parameter_set_index in parameter_sets.size():
			...
			_static_sets_by_index[parameter_set_index] = _is_static_parameter_set(
				parameter_set, property_names
			)
			_test_case_names_cache.append(
				GdUnitTestParameterSetResolver._build_test_case_name(
					test_case, parameter_set, parameter_set_index
				)
			)
...
func _extract_test_names_by_reflection(test_case: _TestCase) -> PackedStringArray:
	...
	for index in parameter_sets.size():
		test_case_names.append(
			GdUnitTestParameterSetResolver._build_test_case_name(
				test_case, str(parameter_sets[index]), index
			)
		)
	return test_case_names


static func _build_test_case_name(
	test_case: _TestCase, test_parameter: String, parameter_set_index: int
) -> String:
	...
	return (
		...
		% [
			test_case.get_name(),
			parameter_set_index,
			test_parameter.replace("\t", "").replace('"', "'").replace("&'", "'")
		]
	)
...
func load_parameter_sets(test_case: _TestCase, do_validate := false) -> Array:
	...
	var source_code := CLASS_TEMPLATE.replace("${clazz_path}", source_script.resource_path).replace(
		"${test_params}", parameter_arg.value_as_string()
	)
	...
	if parameter_sets.size() != _test_case_names_cache.size():
		...
		GdUnitThreadManager.get_current_context().get_execution_context().add_report(
			GdUnitReport.new().create(GdUnitReport.INTERUPTED, test_case.line_number(), error)
		)
		test_case.skip(true, error)
		test_case._interupted = true
...
func fixure_typed_parameters(
	parameter_sets: Array, arg_descriptors: Array[GdFunctionArgument]
) -> Array:
	for parameter_set_index in parameter_sets.size():
		...
		for parameter_index in parameter_set.size():
			...
			if parameter is Array:
				...
				if arg_descriptor.is_typed_array():
					parameter_set[parameter_index] = Array(
						as_array, arg_descriptor.type_hint(), "", null
					)
	return parameter_sets
