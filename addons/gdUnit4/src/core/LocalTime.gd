...
enum TimeUnit {
	DEFAULT = 0, MILLIS = 1, SECOND = 2, MINUTE = 3, HOUR = 4, DAY = 5, MONTH = 6, YEAR = 7
}
	...
	if local_time_._hour > 0:
		return (
			...
			% [
				...
				local_time_._minute,
				local_time_._second,
				local_time_._millisecond
			]
		)
	if local_time_._minute > 0:
		return (
			"%dmin %ds %dms" % [local_time_._minute, local_time_._second, local_time_._millisecond]
		)
