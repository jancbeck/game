...
const DOUBLER_TEMPLATE: GDScript = preload(
	"res://addons/gdUnit4/src/core/GdUnitObjectInteractionsTemplate.gd"
)
...
static func check_leaked_instances() -> void:
	...
	for key in Engine.get_meta_list():
		if key.begins_with(DOUBLER_INSTANCE_ID_PREFIX):
			...
			push_error(
				(
					"GdUnit internal error: an spy/mock instance '%s', class:'%s' is not removed from the engine and will lead in a leaked instance!"
					% [instance, instance.__SOURCE_CLASS]
				)
			)
...
static func load_template(
	template: String, class_info: Dictionary, instance: Object
) -> PackedStringArray:
	# store instance id
	var source_code := (
		template
		. replace(
			"${instance_id}", "%s%d" % [DOUBLER_INSTANCE_ID_PREFIX, abs(instance.get_instance_id())]
		)
...
static func double_functions(
	instance: Object,
	clazz_name: String,
	clazz_path: PackedStringArray,
	func_doubler: GdFunctionDoubler,
	exclude_functions: Array
) -> PackedStringArray:
	var doubled_source := PackedStringArray()
	var parser := GdScriptParser.new()
	var exclude_override_functions := (
		EXCLUDE_VIRTUAL_FUNCTIONS + EXCLUDE_FUNCTIONS + exclude_functions
	)
	var functions := Array()
	...
	if not ClassDB.class_exists(clazz_name):
		...
		for func_descriptor in class_descriptor.functions():
			if instance != null and not instance.has_method(func_descriptor.name()):
				...
				continue
			if (
				functions.has(func_descriptor.name())
	...
	for method: Dictionary in clazz_functions:
		...
		if instance != null and not instance.has_method(func_descriptor.name()):
			...
			continue
		functions.append(func_descriptor.name())
		doubled_source.append_array(
			func_doubler.double(func_descriptor, instance is CallableDoubler)
		)
...
static func is_invalid_method_descriptior(method: Dictionary) -> bool:
	...
	if (
		type == TYPE_INT
		and not clazz_name.is_empty()
		and not (usage & PROPERTY_USAGE_CLASS_IS_ENUM)
	):
		return true
