...
class CLIRunner:
	...
	func show_version() -> void:
		_console.prints_color(
			"Godot %s" % Engine.get_version_info().get("string"), Color.DARK_SALMON
		)
		var config := ConfigFile.new()
		config.load("addons/gdUnit4/plugin.cfg")
		_console.prints_color(
			"GdUnit4 %s" % config.get_value("plugin", "version"), Color.DARK_SALMON
		)
		quit(RETURN_SUCCESS)
		...
		if DisplayServer.get_name() == "headless":
			if _headless_mode_ignore:
				...
				(
					...
					. prints_error("Abnormal exit with %d" % RETURN_ERROR_HEADLESS_NOT_SUPPORTED)
				)
	...
	func skip_suites(test_suites: Array[Node], config: GdUnitRunnerConfig) -> void:
		...
		_console.prints_warning(
			"Found excluded test suite's configured at '%s'" % _runner_config_file
		)
		for test_suite in test_suites:
	...
	func skip_suite(test_suite: Node, skipped: Dictionary) -> void:
		...
		var test_suite_path: String = (
			test_suite.get_meta("ResourcePath")
			if test_suite.get_script() == null
				...
				if skipped_tests.is_empty():
					...
					for test_to_skip in skipped_tests:
						...
						if test_case:
							...
							_console.prints_warning(
								"Mark test case '%s':%s as skipped" % [suite_to_skip, test_to_skip]
							)
						else:
							(
								_console
		...
		match event.type():
			...
			GdUnitEvent.STOP:
				...
				(
					_console
					. prints_color(
						build_executed_test_suite_msg(
							_report.suite_executed_count(), _report.suite_count()
						),
						Color.DARK_SALMON
					)
					. prints_color(
						build_executed_test_case_msg(
							_report.test_executed_count(), _report.test_count()
						),
						Color.DARK_SALMON
					)
					. prints_color(
					...
					. prints_color("Open Report at: file://%s" % report_path, Color.CORNFLOWER_BLUE)
				)
			...
			GdUnitEvent.TESTCASE_AFTER:
				...
				_report.add_testcase_reports(
					event.resource_path(), event.test_name(), event.reports()
				)
	...
	func print_status(event: GdUnitEvent) -> void:
		match event.type():
			GdUnitEvent.TESTSUITE_BEFORE:
				_console.prints_color(
					"Run Test Suite %s " % event.resource_path(), Color.ANTIQUE_WHITE
				)
			GdUnitEvent.TESTCASE_BEFORE:
				(
					_console
	...
	func _print_failure_report(reports: Array[GdUnitReport]) -> void:
		for report in reports:
			if (
				...
				_console.prints_color(
					"	Report:", Color.DARK_TURQUOISE, CmdConsole.BOLD | CmdConsole.UNDERLINE
				)
	...
	func _print_status(event: GdUnitEvent) -> void:
		if event.is_flaky() and event.is_success():
			...
			_console.print_color(
				"FLAKY (%d retries)" % retries,
				Color.GREEN_YELLOW,
				CmdConsole.BOLD | CmdConsole.ITALIC
			)
			...
			if retries > 1:
				_console.print_color(
					"FAILED (retry %d)" % retries, Color.FIREBRICK, CmdConsole.BOLD
				)
			else:
